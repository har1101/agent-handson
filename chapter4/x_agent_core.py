...
def ask_human(tool_call: ToolCall):
    ...
    if feedback == "APPROVE": # ユーザーがツール利用を承認した時
        return tool_call
    
    # ユーザーがツール利用を承認しなかった時
    return ToolMessage(
        content="ユーザーによってツール利用が拒否されたため、処理を終了してください。",
        name=tool_call["name"],
